...
class NcsWorker(object):
    ...
    def __init__(self, devid, device, model_xml, frameBuffer, results, camera_width, camera_height, number_of_ncs, vidfps, nPoints, w, h, new_w, new_h,qNice):
        self.devid = devid
    ...
    def predict_async(self):
        ...
        try:
            ...
            if self.exec_net.requests[dev].wait(0) == 0:
                ...
                ################################################
                self.qNice.put({keypointsMapping[i]: detected_keypoints[i] for i in range(len(keypointsMapping))})
                ################################################
...
def inferencer(device, model_xml, results, frameBuffer, number_of_ncs, camera_width, camera_height, vidfps, nPoints, w, h, new_w, new_h,qNice):

    # Init infer threads
    threads = []
    for devid in range(number_of_ncs):
        thworker = threading.Thread(target=async_infer, args=(NcsWorker(devid, device, model_xml, frameBuffer, results, camera_width, camera_height, number_of_ncs, vidfps, nPoints, w, h, new_w, new_h,qNice),))
        thworker.start()
...
def liveGraph(qNice):
    ...
    def animate(i, xs, ys):
        ...
        qNiceNow=qNice.get()
        requiredPoints=['Nose']
        varyNiceDict = { reqKeys: qNiceNow[reqKeys] for reqKeys in requiredPoints } 
        for reqKeys in requiredPoints:
            xpos=(((varyNiceDict[reqKeys][0][0]))  if len(varyNiceDict[reqKeys])>0 else 0)
        ...
        plt.title('Nose Movements over the time')
        plt.ylabel('X direction')
    ani = animation.FuncAnimation(fig, animate, fargs=(xs, ys), interval=100)
    plt.show()

#        varyNiceDict = { reqKeys: qNiceNow[reqKeys] for reqKeys in ['R-Sho', 'R-Elb', 'R-Wr', 'L-Sho', 'L-Elb', 'L-Wr',  'R-Knee', 'R-Ank', 'L-Knee', 'L-Ank', 'R-Eye', 'L-Eye']}
       # print(varyNiceDict)
#########################################
if __name__ == '__main__':
    ...
    try:
        ...
        p = mp.Process(target=inferencer, args=(device, model_xml, results, frameBuffer, number_of_ncs, camera_width, camera_height, vidfps, nPoints, w, h, new_w, new_h,qNice), daemon=True)
        p.start()
        processes.append(p)
        ...
        p = mp.Process(target=liveGraph, args=(qNice,), daemon=True)
        p.start()
        processes.append(p)
